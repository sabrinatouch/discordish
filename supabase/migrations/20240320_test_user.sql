...
BEGIN
  ...
  VALUES (user_id, 'testuser', 'test@example.com', password_hash, 'https://i.pravatar.cc/150?img=1', 'online');

  -- Insert a test server for the user
  INSERT INTO servers (name, owner_id, icon_url, description)
  VALUES ('Test Server', user_id, 'https://i.pravatar.cc/150?img=2', 'A test server for development');

  -- Add the user as a server member
  INSERT INTO server_members (server_id, user_id, role)
  VALUES ((SELECT id FROM servers WHERE name = 'Test Server'), user_id, 'owner');

  -- Insert a test channel
  INSERT INTO channels (server_id, name, type)
  VALUES ((SELECT id FROM servers WHERE name = 'Test Server'), 'general', 'text');

  -- Insert a test message
  INSERT INTO messages (channel_id, user_id, content, created_at)
  VALUES (
    (SELECT id FROM channels WHERE name = 'general' AND server_id = (SELECT id FROM servers WHERE name = 'Test Server')),
    user_id,
